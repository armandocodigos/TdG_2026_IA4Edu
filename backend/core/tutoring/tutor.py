...
PROMPT_TUTOR = """
Eres un tutor de matematicas conversacional, paciente y cercano.
Tu estilo es el de un companero que acompana sin resolver por el estudiante.

ESTADO ACTUAL: {estado}
INSTRUCCION: {instruccion}

ESTADO PEDAGOGICO: {estado_pedagogico}
AJUSTE PEDAGOGICO: {instruccion_pedagogica}
...
5. Si el estudiante mostro una idea parcial, parte de esa idea.
6. No repitas preguntas del historial; si ya preguntaste algo parecido,
   haz una pregunta mas especifica.
7. No uses tono de libro de texto ni preguntas filosoficas largas.
8. No uses elogios exagerados como "excelente", "perfecto" o "brillante".
   Usa como maximo: "Bien", "Vas encaminado" o "Esa idea ayuda".
...
11. En trigonometria/precalculo, orienta con una pista pequena,
    no con el procedimiento completo.

EJEMPLOS DE TONO:
- "Vas encaminado. Que operacion te ayudaria con el 5?"
- "Esa idea ayuda. Como escribirias la sustitucion con x = 5?"
- "Mira solo el signo. Estas seguro de esa parte?"
- "Ya diste un resultado. Como podrias comprobarlo?"
    ...
    if (
        ...
        if estado == "EJERCICIO_FUNCION":
            return (
                ...
                "la sustitucion que estas haciendo?"
            )

        if estado == "EJERCICIO_DOMINIO_FUNCION":
            return (
                "Ya estamos mirando dominio; ves algun "
                "denominador, raiz par o logaritmo que limite x?"
            )

        if estado == "EJERCICIO_TRIG":
            return (
                "Ya preguntamos eso; mira la razon trigonometrica. "
                "Que lado o angulo estas usando?"
            )

        return (
            "Ya vimos ese punto; que parte concreta "
...
def function_correction_fallback(
    ...
    if pedagogical_state in (
        ...
        return (
            "Mira la sustitucion: en "
            f"{extract_function_evaluation(mensaje)}, "
            "que deberia pasar con la x?"
        )
    ...
    if not focus:
        ...
        return (
            "Ese resultado no cuadra todavia. "
            f"Como escribirias {extract_function_evaluation(mensaje)} "
            "reemplazando x?"
        )

    return (
        f"Ese resultado no cuadra todavia. Mira primero {focus}. "
        "Que te da esa parte antes de seguir?"
    )
...
def sanitize_tutor_response(
    ...
    if pedagogical_state == "cambio_contexto":
        ...
        if "algebra" in message_lower:
            return (
                ...
                "Que tipo de algebra quieres practicar?"
            )

        if "funcion" in message_lower:
            return (
                "De acuerdo, cambiamos a funciones. "
                "Que funcion o idea quieres revisar?"
            )
        ...
        if (
            ...
            return (
                "De acuerdo, cambiamos a limites. "
                "Tienes un limite concreto para revisar?"
            )

        return (
            "De acuerdo, dejamos el ejercicio anterior. "
            "Que tema quieres trabajar ahora?"
        )
    ...
    if estado == "EXPLORAR":
        ...
        if "functions" in message_lower or "funcion" in message_lower:
            return (
                ...
                "Tienes una funcion concreta o quieres que proponga una?"
            )
    ...
    if (
        ...
        if (
            ...
            return (
                "Vas encaminado. "
                "Que cuenta pequena harias ahora?"
            )
    ...
    if (
        ...
        if "f(" in (
            ...
            return (
                "Bien, esa evaluacion queda revisada. "
    ...
    if pedagogical_state == "solucion_parcial":
        ...
        return (
            "Vas encaminado. "
            "Esa es la unica solucion posible?"
        )
    ...
    if (
        ...
        if re.search(
            ...
            return (
                ...
                "En cual ecuacion la sustituirias?"
            )
        ...
        if re.search(
            ...
            return (
                "Bien, esa sustitucion esta planteada. "
                "Como la simplificarias?"
            )
        ...
        if re.search(
            ...
            return (
                "Bien, ya tienes una ecuacion en una variable. "
                "Que operacion haria falta para aislarla?"
            )
        ...
        return (
            "Bien, esa ecuacion sale del sistema. "
            "Que harias con ella ahora?"
        )
    ...
    if (
        ...
        if re.search(
            ...
            return (
                "Mira solo esa division. "
                "Que signo queda al dividir un negativo entre un negativo?"
            )
        ...
        if re.search(
            ...
            if "primera" in message_lower:
                ...
                return (
                    "Mira la primera ecuacion. "
                    "Que pasa con el otro termino al pasarlo de lado?"
                )

            if "segunda" in message_lower:

                return (
                    "Mira la segunda ecuacion. "
                    "Que signo queda al despejar esa variable?"
                )

            return (
                "Mira el despeje de esa variable. "
                "Que termino cambio de lado y con que signo?"
            )
        ...
        if re.search(
            ...
            return (
                "Mira el signo del termino con la variable. "
                "Que signo queda al simplificar?"
            )
        ...
        if "(x,y)" in message_lower or (
            ...
            return (
                "Ese par no satisface el sistema todavia. "
                "Que pasa al probarlo en la segunda ecuacion?"
            )

        if "y =" in message_lower or "x =" in message_lower:

            return (
                "Ese valor no cuadra todavia. "
                "Que obtienes al probarlo en la ecuacion anterior?"
            )

        return (
            "Revisa esa sustitucion en el sistema. "
            "Que igualdad queda al simplificar?"
        )
    ...
    if estado == "EJERCICIO_DOMINIO_FUNCION":
        ...
        if message_claims_invalid_all_real_interval(
            ...
            return (
                ...
                "hacia que infinito deberia ir el otro extremo?"
            )
        ...
        if message_claims_all_real_interval(
            ...
            return (
                ...
                "Quieres revisar rango o pasar a limites?"
            )
        ...
        if message_claims_no_domain_restrictions(
            ...
            return (
                "Esa idea va bien: no aparece una restriccion clara. "
                "Como escribirias ese dominio en intervalo?"
            )
        ...
        if message_claims_limited_domain(
            ...
            return (
                "Revisa de donde sale ese limite. "
                "Hay denominador, raiz par o logaritmo que restrinja x?"
            )

        if "dominio" in message_lower:

            return (
                "Para dominio, busca si la formula prohibe algun x. "
                "Ves denominador, raiz par o logaritmo?"
            )
        ...
        if (
            ...
            return (
                "Para dominio, mira si la formula prohibe algun x. "
                "Ves denominador, raiz par o logaritmo?"
            )
        ...
        if (
            ...
            return (
                ...
                "Ves alguna restriccion para x en la formula?"
            )
